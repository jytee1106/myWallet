...
class InOutLogAdapter  internal constructor(context: Context)
    : RecyclerView.Adapter<InOutLogAdapter.InOutLogViewHolder>(){
    //Declare local variable for Adapter
    private val inflater =  LayoutInflater.from(context)
    private var inOutLogList = emptyList<InOutLog>()

    override fun onCreateViewHolder(parent: ViewGroup,
                                    viewType: Int): InOutLogViewHolder {
        val itemView = inflater.inflate(R.layout.recycleview_item,
            parent,
            false)
    ...
    override fun onBindViewHolder(holder: InOutLogViewHolder, position: Int) {
        ...
        if (inOutLog.type == 1){ //Income
            ...
            holder.textViewType.text = "Expenses"
        }
        holder.textViewAmount.text = inOutLog.amount.toString()
        holder.textViewDate.text = SimpleDateFormat("yyyy.MM.dd.HH:MM").format(inOutLog.created_at)
    }
