    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if(requestCode == REQUEST_ADD){
            if (resultCode == Activity.RESULT_OK){
                ...
                val _type = data?.getIntExtra(AddActivity.TYPE,0)
                val inOutLog = InOutLog(id=0,amount=_amount!!.toFloat(),type=_type!!)

                //TODO: Insert data to database using DAO
                inOutLogViewModel.inserting(inOutLog)
            }
        }
    }
